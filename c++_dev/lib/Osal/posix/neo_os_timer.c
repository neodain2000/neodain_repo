...
****************************************************************************************/
void  NEO_OS_TimespecToUsec(struct timespec time_spec, uint32 *usecs);
void  NEO_OS_UsecToTimespec(uint32 usecs, struct timespec *time_spec);
...
** NEO_OS_MAX_TIMERS should not be set to more than this number.
*/
#define NEO_OS_STARTING_SIGNAL  (SIGRTMAX-1)
...
typedef struct {
    ...
    uint32              accuracy;
    NEO_OS_TimerCallback_t  callback_ptr;
    timer_t              host_timerid;
} NEO_OS_timer_internal_record_t;


/****************************************************************************************
                                   GLOBAL DATA
****************************************************************************************/
NEO_OS_timer_internal_record_t NEO_OS_timer_table[NEO_OS_MAX_TIMERS];
uint32              os_clock_accuracy;

/*
** The Mutex for protecting the above table
*/
pthread_mutex_t     NEO_OS_timer_table_mut;
...
int32 NEO_OS_TimerAPIInit(void)
{
    ...
    */
    ...
    {
        NEO_OS_timer_table[i].free      = TRUE;
        NEO_OS_timer_table[i].creator   = UNINITIALIZED;
        strcpy(NEO_OS_timer_table[i].name,"");
    }
    ...
    */
    ...
    else
    {
        ...
        */
        NEO_OS_TimespecToUsec(clock_resolution, &os_clock_accuracy);

        /*
        ** Create the Timer Table mutex
        */
        status = pthread_mutex_init((pthread_mutex_t *) &NEO_OS_timer_table_mut, NULL);
        if (status < 0)
        {
            return_code = NEO_OS_ERROR;
        }
...
*/
void NEO_OS_TimerSignalHandler(int signum)
{
	uint32  timer_id;

	timer_id = NEO_OS_STARTING_SIGNAL - signum;

	if (timer_id  < NEO_OS_MAX_TIMERS)
	{
		if (NEO_OS_timer_table[timer_id].free == FALSE)
		{
			(NEO_OS_timer_table[timer_id].callback_ptr) (timer_id);
		}
	}
}
 
/******************************************************************************
 **  Function:  NEO_OS_UsecToTimespec
 **
...
/******************************************************************************
 **  Function:  NEO_OS_TimespecToUsec
 **
...
int32 NEO_OS_TimerCreate(uint32 *timer_id, const char *timer_name, uint32 *clock_accuracy, NEO_OS_TimerCallback_t callback_ptr)
{
   ...
   */
   if (strlen(timer_name) > NEO_OS_MAX_API_NAME)
   {
   ...
   ** Check Parameters 
   */
   NEO_OS_InterruptSafeLock(&NEO_OS_timer_table_mut, &mask, &previous);
    
   for(possible_tid = 0; possible_tid < NEO_OS_MAX_TIMERS; possible_tid++)
   {
      if (NEO_OS_timer_table[possible_tid].free == TRUE)
         break;
   }


   if( possible_tid >= NEO_OS_MAX_TIMERS || NEO_OS_timer_table[possible_tid].free != TRUE)
   {
        NEO_OS_InterruptSafeUnlock(&NEO_OS_timer_table_mut, &previous);
   ...
   */
   for (i = 0; i < NEO_OS_MAX_TIMERS; i++)
   {
       if ((NEO_OS_timer_table[i].free == FALSE) &&
            strcmp ((char*) timer_name, NEO_OS_timer_table[i].name) == 0)
       {
            NEO_OS_InterruptSafeUnlock(&NEO_OS_timer_table_mut, &previous);
   ...
   */
   ...
   {
      NEO_OS_InterruptSafeUnlock(&NEO_OS_timer_table_mut, &previous);
      return NEO_OS_TIMER_ERR_INVALID_ARGS;
   }    
   ...
   */
   NEO_OS_timer_table[possible_tid].free = FALSE;
   NEO_OS_InterruptSafeUnlock(&NEO_OS_timer_table_mut, &previous);

   NEO_OS_timer_table[possible_tid].creator = NEO_OS_FindCreator();
   strncpy(NEO_OS_timer_table[possible_tid].name, timer_name, NEO_OS_MAX_API_NAME);
   NEO_OS_timer_table[possible_tid].start_time = 0;
   NEO_OS_timer_table[possible_tid].interval_time = 0;
    
   NEO_OS_timer_table[possible_tid].callback_ptr = callback_ptr;
   ...
   */
   ...
   sig_act.sa_handler = NEO_OS_TimerSignalHandler;
   sig_act.sa_flags = SA_RESTART; 

   memset((void *)&evp, 0, sizeof(evp));
   evp.sigev_notify = SIGEV_SIGNAL; 
   evp.sigev_signo = NEO_OS_STARTING_SIGNAL - possible_tid;

   /*
   ** Create the timer // Reference : man page
   */
   status = timer_create(CLOCK_REALTIME, &evp, (timer_t *)&(NEO_OS_timer_table[possible_tid].host_timerid));
   if (status < 0) 
   {
      NEO_OS_timer_table[possible_tid].free = TRUE;
      return ( NEO_OS_TIMER_ERR_UNAVAILABLE);
   }
   
   /*
   ** Set the signal action for the timer
   */
   sigaction(NEO_OS_STARTING_SIGNAL - possible_tid, &(sig_act), 0); 
...
int32 NEO_OS_TimerSet(uint32 timer_id, uint32 start_time, uint32 interval_time)
{
   ...
   */
   if (timer_id >= NEO_OS_MAX_TIMERS || NEO_OS_timer_table[timer_id].free == TRUE)
   {
   ...
   */
   NEO_OS_timer_table[timer_id].start_time = start_time;
   NEO_OS_timer_table[timer_id].interval_time = interval_time;

   /*
   ** Convert from Microseconds to timespec structures
   */
   NEO_OS_UsecToTimespec(start_time, &(timeout.it_value));
   NEO_OS_UsecToTimespec(interval_time, &(timeout.it_interval));
	
   /*
   ** Program the real timer
   */
   status = timer_settime((timer_t)(NEO_OS_timer_table[timer_id].host_timerid), 
                             0,              /* Flags field can be zero */
                             &timeout,       /* struct itimerspec */
		             NULL);         /* Oldvalue */
   if (status < 0) 
   {
      return ( NEO_OS_TIMER_ERR_INTERNAL);
   }
...
int32 NEO_OS_TimerDelete(uint32 timer_id)
{
   ...
   ** Delete the timer 
   */
   status = timer_delete((timer_t)(NEO_OS_timer_table[timer_id].host_timerid));
   NEO_OS_timer_table[timer_id].free = TRUE;
   if (status < 0)
   {
      return ( NEO_OS_TIMER_ERR_INTERNAL);
   }
...
/***********************************************************************************
**
**    Name: NEO_OS_TimerGetIdByName
**
**    Purpose: This function tries to find a Timer Id given the name 
**             The id is returned through timer_id
**
**    Returns: NEO_OS_INVALID_POINTER if timer_id or timer_name are NULL pointers
...
int32 NEO_OS_TimerGetIdByName (uint32 *timer_id, const char *timer_name)
{
    ...
    */
    ...
    for (i = 0; i < NEO_OS_MAX_TIMERS; i++)
    {
        if (NEO_OS_timer_table[i].free != TRUE &&
                (strcmp (NEO_OS_timer_table[i].name , (char*) timer_name) == 0))
        {
            *timer_id = i;
            return NEO_OS_SUCCESS;
        }
...
}/* end NEO_OS_TimerGetIdByName */

/***************************************************************************************
**    Name: NEO_OS_TimerGetInfo
**
...
**             
**    Returns: NEO_OS_ERR_INVALID_ID if the id passed in is not a valid timer 
**             NEO_OS_INVALID_POINTER if the timer_prop pointer is null
**             NEO_OS_SUCCESS if success
*/
int32 NEO_OS_TimerGetInfo (uint32 timer_id, NEO_OS_timer_prop_t *timer_prop)  
{
    ...
    */
    ...
    timer_prop ->creator       = NEO_OS_timer_table[timer_id].creator;
    strcpy(timer_prop-> name, NEO_OS_timer_table[timer_id].name);
    timer_prop ->start_time    = NEO_OS_timer_table[timer_id].start_time;
    timer_prop ->interval_time = NEO_OS_timer_table[timer_id].interval_time;
    timer_prop ->accuracy      = NEO_OS_timer_table[timer_id].accuracy;
    
    NEO_OS_InterruptSafeUnlock(&NEO_OS_timer_table_mut, &previous);

    return NEO_OS_SUCCESS;
    
} /* end NEO_OS_TimerGetInfo */

/****************************************************************
 * TIME BASE API
 *
 * This is not implemented by this OSAL, so return "NEO_OS_ERR_NOT_IMPLEMENTED"
